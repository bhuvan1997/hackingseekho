...
class Database extends CI_Model
{
    ...
    public function get_enroll($session)
    {
        $sql = "SELECT tbl_batch_enroll.*, tbl_batch.id AS bid, tbl_batch.batch_name,tbl_batch.batch_type,tbl_batch.batch_topic,tbl_batch.batch_desc,tbl_batch.start_date,tbl_batch.end_date,tbl_batch.end_date,tbl_batch.duration,tbl_batch.zoom_link, tbl_user.name AS name FROM tbl_batch_enroll LEFT JOIN tbl_batch ON tbl_batch.id = tbl_batch_enroll.batch_id LEFT JOIN tbl_user ON tbl_user.id = tbl_batch.mentor_id  WHERE tbl_batch_enroll.user_id = '".$session['loggedId']."'";
        $query = $this->db->query($sql);
        return $query->result();
    }

    public function get_notes($session)
    {
        $sql = "SELECT tbl_attachment.*, tbl_batch.batch_name AS btch_n  FROM tbl_attachment LEFT JOIN tbl_batch_enroll ON tbl_batch_enroll.batch_id = tbl_attachment.enroll_id LEFT JOIN tbl_batch ON tbl_batch.id = tbl_attachment.enroll_id WHERE tbl_batch_enroll.user_id = '" . $session['loggedId'] . "'";
        $query = $this->db->query($sql);
        return $query->result();
    }
}
